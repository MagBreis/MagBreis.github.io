function validarDataNasc() {
    ...
    if (idade < 0 || idade > 115) { // checa se idade é negativa ou maior que 130
...
function exibirInfo() {
    ...
    if (ok) {
        divInfo.innerHTML = `Olá ${inputNome}! <br> Seu login é: ${inputEmail} <br> Você tem ${idadeUsuario} anos de idade <br> Você pode usar ${cpfUsuario} como senha.`;
    }
    else {
...
function exibirInfo() {
    ...
    if (ok) {
        divInfo.innerHTML = `Olá ${inputNome}! <br> Seu login é: ${inputEmail} <br> Você tem ${idadeUsuario} anos de idade <br> Se define como uma pessoa do sexo: <br> Você pode usar ${cpfUsuario} como senha.`;
    }
    else {
        alert("Dados inválidos, por favor preencha corretamente.");
    }
